 ...
 */
export async function rateLimit(ip, endpoint = 'global') {
  ...
  const count = await collection.countDocuments({ ip, endpoint, timestamp: { $gte: windowStart } });

  if (count >= MAX_REQUESTS_PER_WINDOW) {
    const oldestEntry = await collection.findOne({ ip, endpoint }, { sort: { timestamp: 1 } });
  ...
  // Record this request
  await collection.insertOne({ ip, endpoint, timestamp: now });
